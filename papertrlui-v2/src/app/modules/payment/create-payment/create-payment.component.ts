...
export class CreatePaymentComponent implements OnInit, OnDestroy {
  ...
  submitPayment(status) {
    ...
    transactionList.forEach(value => {
      const transactionObj = {
        ...
        docNo: null, disc: undefined, fundingAccountId: null
      };
      ...
      transactionObj.fundingAccountId = value.fundingAccountId;
  ...
  paymentTypeChange(fromHeader, data) {
    if (this.f.isOnline.value === this.paymentTypeOnline) {
      if (data.supportedPaymentTypeList) {
        const paymentType = data.supportedPaymentTypeList.find(type => type.id === data['doc.txnType']);
        if (paymentType) {
          data.fundingAccountId = paymentType.otherId;
        }
      }
  ...
  getFundingAccountName(data) {
    if (this.paymentForm.get('isOnline').value) {
      try {
        return data.supportedPaymentTypeList.find(x => x.id === data['doc.txnType']).fundingAccount;
      } catch (e) {
        return '-';
      }
    } else {
