...
@Component({
  selector: 'app-bill-summary-table',
  templateUrl: './bill-summary-table.component.html',
  styleUrls: ['./bill-summary-table.component.scss']
})
export class BillSummaryTableComponent implements OnInit, OnDestroy {
  toDate: any = '';
  fromDate: any = '';
   ...
   */
    ...
    sessionStorage.removeItem(AppTableKeysData.BILL_TABLE_KEY);
    this.gridService.getTableStructure(AppConstant.GRID_PORTAL_BILL_DASHBOARD).subscribe((res: any) => {
  ...
  getDataFromBackend(fromDate, toDate) {
    this.portalDashboardService.getBillTableData(fromDate, toDate).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
  ...
  onMultiSelectChange(event: any, field: any) {
    ...
    this.tableSupportBase.dataSource.forEach((value) => {
      event.value.forEach((value1) => {
        if (value['pBill.subAccount'] === value1) {
