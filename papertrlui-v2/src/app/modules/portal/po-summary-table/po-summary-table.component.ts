...
@Component({
  selector: 'app-po-summary-table',
  templateUrl: './po-summary-table.component.html',
  styleUrls: ['./po-summary-table.component.scss']
})
export class PoSummaryTableComponent implements OnInit, OnDestroy {
  toDate: any = '';
  fromDate: any = '';
   ...
   */
    ...
    sessionStorage.removeItem(AppTableKeysData.PO_TABLE_KEY);
    this.gridService.getTableStructure(AppConstant.GRID_PORTAL_PURCHASE_ORDER_DASHBOARD).subscribe((res: any) => {
  ...
  getDataFromBackend(fromDate, toDate) {
    this.portalDashboardService.getPoTableData(fromDate, toDate).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
  ...
  onMultiSelectChange(event: any, field: any) {
    ...
    this.tableSupportBase.dataSource.forEach((value) => {
      event.value.forEach((value1) => {
        if (value['pPO.subAccount'] === value1) {
