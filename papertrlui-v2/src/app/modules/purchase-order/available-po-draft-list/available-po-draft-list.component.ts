...
@Component({
  selector: 'app-available-po-draft-list',
  templateUrl: './available-po-draft-list.component.html',
  styleUrls: ['./available-po-draft-list.component.scss']
})
export class AvailablePoDraftListComponent implements OnInit {
  ...
  changeStateOfExpenseDraftList() {
    this.drawerService.changeDefaultDrawerState(AppConstant.PO_DRAFT_LIST_MODAL)
      .subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
          this.poUtility.showPoDraftListByDefault = !this.poUtility.showPoDraftListByDefault;
        } else {
  ...
  getDataPatchingStatusFromDraft() {
    this.poService.isProcessingPatchingDataFromPoDraft.subscribe(draftStatus => {
      if (draftStatus) {
