...
export class AccountPendingComponent implements OnInit, OnDestroy {
   ...
   */
  ngOnDestroy(): void {
    sessionStorage.removeItem(AppTableKeysData.ACCOUNT_SYNC_PENDING_TABLE_KEY);
  }
  ...
  onTableChanged(event?) {
    const promise = this.tableSupportBase.tableChanged(this.table, AppTableKeysData.ACCOUNT_SYNC_PENDING_TABLE_KEY);
    promise.then(result => {
      this.tableSupportBase.tableDataOptions = result;
      this.tableSupportBase.tableDataOptions.gridName = this.appConstant.GRID_ACCOUNT_SYNC_PENDING_LIST;
  ...
  ngOnInit() {
    ...
    this.availableHeaderActions.push(AppTableHeaderActions.SYNC);
   ...
   */
    ...
    sessionStorage.removeItem(AppTableKeysData.ACCOUNT_SYNC_PENDING_TABLE_KEY);
    this.gridService.getTableStructure(this.appConstant.GRID_ACCOUNT_SYNC_PENDING_LIST).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
          this.tableSupportBase.tableDataProcess(this.table, res, AppTableKeysData.ACCOUNT_SYNC_PENDING_TABLE_KEY, this.columnSelect);
        } else {
          this.notificationService.errorMessage(res.body.message);
        }
      }, (error => {
  ...
  getDataFromBackend() {
    this.accountSyncService.getPendingAccountSync(this.tableSupportBase.searchFilterDto, this.systemId).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
  ...
  syncButtonClick(val) {
    ...
    this.accountSyncService.sync(val.id, this.systemId).subscribe((res: any) => {
      if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
        this.tableSupportBase.rows = [];
        this.getDataFromBackend();
        this.notificationService.successMessage(HttpResponseMessage.P_SYNC_SUCCESSFULLY);
      } else {
        this.notificationService.errorMessage(res.body.message);
      }
    }, error => {
  ...
  syncBulk() {
    if (this.tableSupportBase.rows.length > 0) {
      ...
      this.accountSyncService.bulkSync(ids, this.systemId).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
          this.tableSupportBase.rows = [];
          this.getDataFromBackend();
          this.notificationService.successMessage(HttpResponseMessage.P_BULK_SYNC_SUCCESSFULLY);
        } else {
          this.notificationService.errorMessage(res.body.message);
        }
      }, error => {
