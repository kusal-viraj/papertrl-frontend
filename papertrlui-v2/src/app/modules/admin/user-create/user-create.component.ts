...
export class UserCreateComponent implements OnInit, AfterViewInit {
    ...
    if (this.editView || this.detailView) {
      this.userService.getUser(this.id).subscribe((res: any) => {
        if (AppResponseStatus.STATUS_SUCCESS === res.status) {
          ...
          this.acceptedPaymentTypeSelected();
  ...
  addAcceptedPaymentTypes(data: any) {
    const paymentTypeGroup = this.formBuilder.group({
    ...
    paymentTypeGroup.get('paymentTypeId').patchValue(data.id);
    paymentTypeGroup.get('name').patchValue(data.name);
    paymentTypeGroup.get('selected').patchValue(false);
    this.acceptedPaymentTypes.push(paymentTypeGroup);
  }
  ...
  setPreferredPaymentType() {
    ...
    if (this.selectedPaymentTypes.data.length){
      this.createUserForm.get('preferredPaymentTypeId').setValidators(Validators.required);
    } else {
      this.createUserForm.get('preferredPaymentTypeId').setValidators(null);
    }
    this.createUserForm.get('preferredPaymentTypeId').updateValueAndValidity();
  ...
  get fieldValueAccountNumber() {
    return (this.createUserForm.get('accountNumber'));
  }

  get fieldValueAccountRoutingNumber() {
    return (this.createUserForm.get('accountRoutingNumber'));
  }
