...
export class DiscountTableComponent implements OnInit, OnDestroy {
  ...
  public isOnlyClosedDetailView = false;
  ...
  constructor(public billsService: BillsService, public gridService: GridService, public notificationService: NotificationService,
              public dashboardService: DashboardService, public confirmationService: ConfirmationService,
              public bulkNotificationDialogService: BulkNotificationDialogService, public detailViewService: DetailViewService,
              public router: Router, public privilegeService: PrivilegeService) {
  }
   ...
   */
  ngOnDestroy(): void {
    sessionStorage.removeItem(AppTableKeysData.BILL_DISCOUNT_TABLE_KEY);
  ...
  loadTableData() {
    ...
    sessionStorage.removeItem(AppTableKeysData.BILL_DISCOUNT_TABLE_KEY);
    this.gridService.getTableStructure(this.appConstant.GRID_DASHBOARD_DISCOUNT_LIST).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
          this.tableSupportBase.tableDataProcess(this.table, res, AppTableKeysData.BILL_DISCOUNT_TABLE_KEY, this.columnSelect);
        } else {
      ...
      },
      {
        ...
        command: () => {
          this.billsService.getAuditTrial(this.activeAction.id).subscribe((res: any) => {
            this.billNo = JSON.parse(JSON.stringify(this.activeAction))['discount.billNo'];
   ...
   * @param status statua
   */
  actionButtonList(status, billType, billObject) {
    return this.tableActionList.filter(this.isActionMatch(status, billType,
      AppEnumConstants.STATUS_COMMON, this.isValidApproveAccess(billObject)));
  }
  ...
  getDataFromBackend() {
    this.dashboardService.getBillTableData(this.tableSupportBase.searchFilterDto).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
  ...
  onTableChanged(event?) {
    const promise = this.tableSupportBase.tableChanged(this.table, AppTableKeysData.BILL_DISCOUNT_TABLE_KEY);
    promise.then(result => {
      this.tableSupportBase.tableDataOptions = result;
      this.tableSupportBase.tableDataOptions.gridName = this.appConstant.GRID_DASHBOARD_DISCOUNT_LIST;
