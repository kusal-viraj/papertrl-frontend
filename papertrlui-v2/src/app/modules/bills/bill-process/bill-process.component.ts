...
@Component({
  ...
  styleUrls: ['./bill-process.component.scss']
})

export class BillProcessComponent extends MandatoryFields implements OnInit, OnDestroy {
  ...
  constructor(public router: Router, public formBuilder: UntypedFormBuilder, public billSubmitService: BillSubmitService,
              ...
              public creditNoteService: CreditNoteService, private elementRef: ElementRef, public manageFeatureService: ManageFeatureService,
              public privilegeService: PrivilegeService, public sanitizer: DomSanitizer, public billsService: BillsService,
              public poService: PoService, private billSocketService: BillSocketService,
  ...
  ngOnInit(): void {
    ...
    this.billSocketService.connect();
    this.initExpense();
    this.initAddItems();
    this.getVendorList();
  ...
  changedDepartment(event: any, dpNameDepartment) {
    this.commonUtil.patchHeaderDepartmentToLineLevel(this.reviewBillDetailsForm, -1, false,null, true, true);
  ...
  resetReviewBillDetailsForm(fromSelectBill) {
    this.gaService.trackScreenButtonEvent(
      ...
      AppAnalyticsConstants.RESET_BUTTON,
      AppAnalyticsConstants.PROCESS_BILL_SCREEN,
    );
    ...
    return new Promise<void>(async resolve => {
      ...
      this.clearAutomation();
      this.reviewBillDetailsForm.get('event').patchValue(AppDocuments.DOCUMENT_EVENT_SUBMITTED);
      if (!fromSelectBill) {
        await this.initItemAccountDetail(this.billDetail.billItemCostDistributions, this.billDetail.billExpenseCostDistributions);
  ...
  async billSelected(bill) {
    ...
    this.billSubmitService.getBillDetail(id, false).then((res: any) => {
      ...
      this.ocrRunningStatus = res.body.ocrRunningStatus;
      ...
      this.reviewBillDetailsForm.get('isSelectedBill').patchValue(true);
  ...
  getVendorRealtedItems(vendorId) {
    return new Promise(async resolve => {
      await this.getVendorRelatedTerms(vendorId);
  ...
  getVendorItemList(selectedVendorId) {
    if (selectedVendorId) {
      this.billsService.getItemListByVendorId(selectedVendorId, true).subscribe((res: any) => {
 ...
 */
   ...
   */
  getPendingBillList() {
    this.isLoadingBillAttachment = true;
    this.billSubmitService.getSubmitPendingInvoices().subscribe((res: any) => {
      if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
        ...
        this.isLoadingBillAttachment = false;
        // this.automationService.setupFocusOutListeners(this.reviewBillDetailsForm, this.renderer, this.el, this.setFieldValueDto);
  ...
  getVendorRelatedTemplateList(vendorId) {
    if (isNotNullOrUndefined(vendorId)) {
      this.billSubmitService.getTemplateListByVendorId(vendorId, true).subscribe((res: any) => {
        if (AppResponseStatus.STATUS_SUCCESS === res.status) {
  ...
  getApprovalUserListAccordingToVendorSelection(listInstance: DropdownDto, vendorId) {
    ...
    this.billsService.getApprovalUserListAccordingToVendorSelection(this.billMasterDto.createdBy, authorities, vendorId, true).subscribe((res: any) => {
      if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
  ...
  submitBill(action, editOnly, isSubmit) {
    this.reviewBillDetailsForm.get('id').patchValue(this.billId);
    ...
    if (this.reviewBillDetailsForm.valid) {
      if (action === 'SUBMIT_FOR_APPROVED') {
        ...
        if (this.billStatus === AppEnumConstants.STATUS_DRAFT) {
          ...
          this.submitBillForApproval(billMasterDto);
        }
  ...
  automaticallySelectBillAfterTriggerEvent() {
    if (this.currentIndex === this.billList.length - 1) {
      this.billList.splice(this.currentIndex, 1);
      this.currentIndex = this.billList.length - 1;
  ...
  dateFormatChanged(dateFormatLabel: Dropdown){
    this.validateDate();
    this.automationService.setAutomationData(this.reviewBillDetailsForm, AppFormConstants.BILL_DATE_FORMAT)
    this.commonUtil.isPressEnterInsideDropdown(dateFormatLabel)
  }
...
*/
  ...
  */
  ...
  getProjectTaskList(listInstance: DropdownDto) {
    this.billsService.getProjectTaskUserWise(AppConstant.PROJECT_CODE_CATEGORY_ID, true).subscribe((res: any) => {
      listInstance.data = res.body;
...
*/
    ...
    return new Promise<void>(resolve => {
      this.additionalFieldService.getAdditionalField(id, isDetailView, true).subscribe((res: any) => {
        if (AppResponseStatus.STATUS_SUCCESS === res.status) {
          ...
          if (this.billStatus === this.enums.STATUS_DRAFT || this.ocrRunningStatus === this.enums.STATUS_YES) {
            this.reviewBillDetailsForm.patchValue(this.billDetail);
            this.automationService.setUpFocusListeners(this.reviewBillDetailsForm, this.renderer, this.el, this.setFieldValueDto, AppDocumentType.BILL, this.automationService.billInputFieldsForAutomation);
            setTimeout(() => {
              this.reviewBillDetailsForm.get(AppFormConstants.PATCH_SET_FIELD_FULL_OBJECT).patchValue(true);
            ...
            this.billDetail.linkedWithCostDistribution && isTotalAmountGreaterThanZero ?
              this.reviewBillDetailsForm.get('distributionCostTotal').patchValue(this.billDetail.billAmount) :
  ...
  public addLineFieldForItem(field: AdditionalFieldDetailDto) {
    if (!this.commonUtil.checkUndefinedLineItemsValues(this.billDetail.billItemCostDistributions, field, false, true)) {
      return;
    }
  ...
  public addLineFieldForExpense(field: AdditionalFieldDetailDto) {
    if (!this.commonUtil.checkUndefinedLineItemsValues(this.billDetail.billExpenseCostDistributions, field, false, true)) {
      return;
    }
  ...
  public get headingSectionArray() {
    return this.reviewBillDetailsForm.get('additionalData') as UntypedFormArray;
  }
  ...
  isViewSubmitForApprovalButton(): boolean {
    return (this.privilegeService.isAuthorized(this.appAuthorities.BILL_CREATE) || this.isSubmissionWorkFlow)
      && !this.isSaveAsApprovedWorkFlow && ((this.billStatus === this.enums.STATUS_APPROVED &&
        this.isEnabledSubmitForApprovalButton) || this.billStatus === AppEnumConstants.STATUS_DRAFT || this.billStatus === this.enums.STATUS_SUCCESS);
  }
  ...
  isSaveAsDraftEnabled() {
    return (this.billStatus === this.enums.STATUS_DRAFT || this.billStatus === this.enums.STATUS_SUCCESS);
  }
   ...
   */
  termManuallyChanged(fromDp?, termName?) {
    ...
    this.reviewBillDetailsForm.get('termManuallyChanged').patchValue(true);
  }
  ...
  getVendorRelatedTerms(id) {
    return new Promise(resolve => {
      if (this.reviewBillDetailsForm.get('termManuallyChanged').value) {
        resolve(true);
        return;
      }
      ...
      this.billsService.getVendorRelatedTerms(id).subscribe({
        next: (res: any) => {
          ...
          if (res.body?.visibilityStatus === AppEnumConstants.STATUS_LETTER_INACTIVE) {
            this.reviewBillDetailsForm.get(AppFormConstants.TERM).patchValue(res.body.id);
            this.reviewBillDetailsForm.get(AppFormConstants.NET_DAYS_DUE).patchValue(res.body.netDaysDue);
            this.reviewBillDetailsForm.get(AppFormConstants.DISCOUNT_PERCENTAGE).patchValue(res.body.discountPercentage);
            this.reviewBillDetailsForm.get(AppFormConstants.DISCOUNT_DAYS_DUE).patchValue(res.body.discountDaysDue);
          } else {
            this.reviewBillDetailsForm.get(AppFormConstants.TERM).patchValue(res.body?.id);
          }
          resolve(true);
  ...
  changeHeaderProjectCode(dpNameProjectTask: Dropdown) {
    ...
    this.billUtility.loadAccountsAccordingToTheProjectTaskId(this.reviewBillDetailsForm.get('projectCodeId').value, null, true, this.billId);
  ...
  changeBillTemplate(selectedTemplateName: Dropdown) {
    this.automationService.setAutomationData(this.reviewBillDetailsForm, AppFormConstants.TEMPLATE_ID)
    this.changeDropDownList('Template', selectedTemplateName.selectedOption.id);
    // this.getTemplateDetectData(this.billId, selectedTemplateName.selectedOption.id);
    this.commonUtil.isPressEnterInsideDropdown(selectedTemplateName);
  }

  changePo(selectedPoName: Dropdown) {
    this.isVisibleNotificationContent = null;
    this.getRemainingCeiling(selectedPoName.selectedOption.id, selectedPoName.selectedOption.name);
    this.reviewBillDetailsForm.get('closePo').patchValue(false);
    this.billUtility.getMatchingTableData(this.reviewBillDetailsForm, this.expenseFields, this.itemForm);
  ...
  projectCodeChangedExp(code: Dropdown, i: any, account) {
    this.automationService.triggerFocusListeners(AppFormConstants.FOCUS_LISTENER);
    this.changeList('ProjectCodesAccount', code.selectedOption.id, i);
    this.billUtility.loadAccountsAccordingToTheProjectTaskId(code.selectedOption.id, i, true, this.billId);
