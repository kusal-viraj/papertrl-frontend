...
export class BillSocketService {
  private webSocketEndPoint = ApiEndPoint.API_URL + '/secured/ocr';
  private getMessage = '/user/secured/receive_ocr_status';
    ...
    try {
      ...
      this.stompClient.connect({},
        (frame) => {
          ...
          // CALL START ENDPOINT //
          this.configBillWebSocketConnection(true);
        }, this.errorCallBack);
    } catch (ex) {
      console.error(ex);
    }
   ...
   */
  configBillWebSocketConnection(status) {
    let client = null;
    ...
    try {
      this.stompClient.send('/secured/config_bill_web_socket_connection', {}, body);
    } catch (ex) {
      console.error(ex);
    }
  }
