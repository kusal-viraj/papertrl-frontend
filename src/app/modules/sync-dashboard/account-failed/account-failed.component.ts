...
@Component({
  ...
  styleUrls: ['./account-failed.component.scss']
})
export class AccountFailedComponent implements OnInit, OnDestroy {
   ...
   */
  ngOnDestroy(): void {
    sessionStorage.removeItem(AppTableKeysData.ACCOUNT_SYNC_FAILED_TABLE_KEY);
  }
  ...
  onTableChanged(event?) {
    const promise = this.tableSupportBase.tableChanged(this.table, AppTableKeysData.ACCOUNT_SYNC_FAILED_TABLE_KEY);
    promise.then(result => {
      this.tableSupportBase.tableDataOptions = result;
      this.tableSupportBase.tableDataOptions.gridName = this.appConstant.GRID_ACCOUNT_SYNC_FAILED_LIST;
  ...
  getDropDown(col: any) {
    switch (col.field) {
      case 'syncDetail.objectType':
        return col.dropdownValues;
      case 'syncDetail.recodeStatus':
        return col.dropdownValues;
      case 'syncDetail.tpCompanyId':
   ...
   */
    ...
    sessionStorage.removeItem(AppTableKeysData.ACCOUNT_SYNC_FAILED_TABLE_KEY);
    this.gridService.getTableStructure(this.appConstant.GRID_ACCOUNT_SYNC_FAILED_LIST).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
          this.tableSupportBase.tableDataProcess(this.table, res, AppTableKeysData.ACCOUNT_SYNC_FAILED_TABLE_KEY, this.columnSelect);
        } else {
          this.notificationService.errorMessage(res.body.message);
        }
      }, (error => {
  ...
  getDataFromBackend() {
    this.accountSyncService.getFailedAccountSync(this.tableSupportBase.searchFilterDto, this.systemId).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
  ...
  syncButtonClick(val) {
    // this.activeAction = val;
    this.accountSyncService.reSync(val.id, this.systemId).subscribe((res: any) => {
      if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
  ...
  syncBulk() {
    if (this.tableSupportBase.rows.length > 0) {
      ...
      this.accountSyncService.bulkReSync(ids, this.systemId).subscribe((res: any) => {
        if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
          this.tableSupportBase.rows = [];
          this.getDataFromBackend();
