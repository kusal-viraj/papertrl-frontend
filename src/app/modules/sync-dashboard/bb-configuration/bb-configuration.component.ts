...
@Component({
  selector: 'app-bb-configuration',
  templateUrl: './bb-configuration.component.html',
  styleUrls: ['./bb-configuration.component.scss']
})
export class BbConfigurationComponent implements OnInit {
  ...
  connectBb(event: any) {
    ...
    if (event.checked) {
      if (this.systemId === AppConstant.SYSTEM_BLACKBAUD) {
        this.connectToBlackBaud(btnCondition);
      }
    } else {
      if (this.systemId === AppConstant.SYSTEM_BLACKBAUD) {
        this.disConnectFromBlackBaud(btnCondition);
      }
    }
  }
   ...
   */
  connectToBlackBaud(btnCondition) {
    this.accountSyncService.getBBOnlineConnectURL().subscribe((res: any) => {
      if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
   ...
   * @param btnCondition button status
   */
  disConnectFromBlackBaud(btnCondition) {
    this.accountSyncService.disconnectFromBb().subscribe((res: any) => {
      if (res.status === AppConstant.HTTP_RESPONSE_STATUS_SUCCESS) {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.connectionTrigger.currentValue === 'BB') {
      this.configurations();
    }
  }
